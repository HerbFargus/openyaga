...
- **`.evb`** (2,884 files, 1.2 MB) — event/lipsync timing blobs read by
  `yagaevents.dll`. They start with a little-endian u32 that tracks file size
  (0x57–0x5B for the ~90-byte ones), then `01 00 00 00`. Each `.mp3` talkie has a
  matching `.evb`, so these are very likely the phoneme timelines that drive the
  masks above — linyaga lists lipsync as its one missing feature.
